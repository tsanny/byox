...
// Compiled for linux
#else
// windows does not define these values as default
#define INFINITY 1e8
...
// origin and direction). We test if the ray intersects with any of the geometry
// inside the scene. If the ray intersects with an object, we compute the
// intersection point, the normal at the intersection point, and shade this
// point using this information.
//
// Shading depends on the surface property (is it transparent, reflective, or
// diffuse?). The function returns a color for the ray if it intersects with an
// object that is the color of the object at the intersection point. Otherwise,
...
Vec3f trace(const Vec3f &rayorig, const Vec3f &raydir,
  ...
  if (!sphere)
    return Vec3f(2);
  Vec3f surfaceColor =
      0; // color of the ray/surfaceof the object intersected by the array
  Vec3f phit = rayorig + raydir * tnear; // point of intersection
  Vec3f nhit = phit - sphere->center;    // normal at the intersection point
  nhit.normalize();                      // normalize normal direction
  // If the normal and the view direction is not the opposite of each other,
  // reverse the normal direction. That also means that we are inside of the
  // sphere, so set the inside bool to true. Lastly, reverse the sign of IdotN
  ...
  if ((sphere->transparency > 0 || sphere->reflection > 0) &&
    ...
    Vec3f refraction = 0;
    // if the sphere is transparent, compute the refraction ray (transmission)
    ...
    // the result is a mix of reflection and refraction (if sphere is
    // transparent)
...
// trace it and return a color. If the ray hits a sphere, we return the color
// of the sphere at the intersection point, else we return the background color.
...
// In the main function, we will create a scene which contains 5 spheres and 1
// light (which is also a sphere). Then, we render the scence using the render()
// function.
int main(int argc, char **argv) {
  ...
  spheres.push_back(
      Sphere(Vec3f(0.0, 0, -20), 4, Vec3f(1.00, 0.1, 0.8), 1, 0.5));
  spheres.push_back(
      Sphere(Vec3f(5.0, -1, -15), 2, Vec3f(0.90, 0.76, 0.46), 1, 0.0));
  spheres.push_back(
      Sphere(Vec3f(5.0, 0, -25), 3, Vec3f(0.65, 0.77, 0.97), 1, 0.0));
  spheres.push_back(
      Sphere(Vec3f(-5.5, 0, -15), 3, Vec3f(0.90, 0.90, 0.90), 1, 0.0)); // light
